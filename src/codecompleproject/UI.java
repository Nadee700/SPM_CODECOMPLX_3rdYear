...
int TCtCount =0;  
    ...
    private void initComponents() {
        ...
        jLabel2.setText("TW");
        jPanel1.add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(790, 980, -1, -1));

        jLabel3.setText("TotalTW");
        jPanel1.add(jLabel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(860, 980, -1, -1));

        jLabel4.setText("Cs");
        jPanel1.add(jLabel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(50, 980, -1, -1));

        jLabel5.setText("TotalCs");
        jPanel1.add(jLabel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(100, 980, -1, -1));

        getContentPane().add(jPanel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 10, 1020, 1090));
    ...
    private void btnSizeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSizeActionPerformed
        ...
        String keywords[] = {"abstract", "assert", "boolean",
            ...
            "synchronized", "this", "transient", "true",
            "volatile", "while", "endl", "\\n"};
        String keywords2[] = {"new", "delete", "throw", "throws"};
        final String regex = "\\+\\+|--|->|::|<<=|<=|>=|>>>=|>>>|<<<|\\|\\||&&|>>|<<|[\\|\\/&%=+!<>^*-]=|[|\\.=,~!*<>+\\-^!%]";
        final String regex2 = "\\\"([^\\\"]*)\\\"";
        final String regex3 = "\\/\\/.*|(?s)\\/\\\\*.*?\\\\*\\/";
        final String regex4 = "\\d+";
        final String regex5 = "\\b(?:(?:auto\\s*|String\\s*|const\\s*|unsigned\\s*|signed\\s*|register\\s*|volatile\\s*|static\\s*|void\\s*|short\\s*|long\\s*|char\\s*|int\\s*|float\\s*|double\\s*|_Bool\\s*|complex\\s*)+)(?:\\s+\\*?\\*?\\s*)([a-zA-Z_][a-zA-Z0-9_]*)\\s*[\\[;,=)]";
        final String regex6 = "((?:(?:public|private|protected|static|final|abstract|synchronized|volatile)\\s+)*)\\s*(\\w+)\\s*(\\w+)\\(.*?\\)\\s*";

        ArrayList<String> names = new ArrayList<String>();

        for (int i = 0; i < lineArr.length; i++) {
            String tempLine = lineArr[i];
            int count = 0;
            //Ignore comments
            final Pattern pattern3 = Pattern.compile(regex3, Pattern.MULTILINE);
            final Matcher matcher3 = pattern3.matcher(lineArr[i]);

            while (matcher3.find()) {
                lineArr[i] = lineArr[i].replace(matcher3.group(0), "");
            }

            //For double quotes
            final Pattern pattern2 = Pattern.compile(regex2, Pattern.MULTILINE);
            final Matcher matcher2 = pattern2.matcher(lineArr[i]);

            while (matcher2.find()) {
                System.out.println(matcher2.group(0) + "double quotes");
                lineArr[i] = lineArr[i].replace(matcher2.group(0), "");
                count++;
            }

            //For names of varibles
//            final Pattern pattern5 = Pattern.compile(regex5, Pattern.CASE_INSENSITIVE | Pattern.COMMENTS);
//            final Matcher matcher5 = pattern5.matcher(lineArr[i]);
//
//            while (matcher5.find()) {
//                names.add(matcher5.group(1));
//            }
//            final Pattern pattern6 = Pattern.compile(regex6, Pattern.DOTALL);
//            final Matcher matcher6 = pattern6.matcher(lineArr[i]);
//
//            if (matcher6.find()) {
//                names.add(matcher6.group(3));
//            }
//
//            for (int n = 0; n < names.size(); n++) {
//                Pattern p = Pattern.compile(names.get(n));
//                Matcher m = p.matcher(lineArr[i]);
//                while (m.find()) {
//                    System.out.println(m.group(0)+"names");
//                    count++;
//                }
//            }
            ...
            for (int m = 0;
                    ...
                    m++) {
                String test = lineArr[i];
                int index = lineArr[i].indexOf(keywords2[m]);
                while (index != -1) {
                    count += 2;
                    lineArr[i] = lineArr[i].substring(index + keywords2[m].length());
                    index = lineArr[i].indexOf(keywords2[m]);
                }
                lineArr[i] = test;
            }
            ...
            final Matcher matcher = pattern.matcher(lineArr[i]);

            while (matcher.find()) {
                System.out.println(matcher.group(0) + "operaters");
                count++;
            }

            //For numeric values
            final Pattern pattern4 = Pattern.compile(regex4, Pattern.MULTILINE);
            final Matcher matcher4 = pattern4.matcher(lineArr[i]);

            while (matcher4.find()) {
                System.out.println(matcher4.group(0) + "numeric values");
                count++;
            }
    ...
    //   Object[] columnNames = { "Code Lines", "Cs Value", "Ctc value","Cnc Value","Ci","TW","Cps","Cr"};
        Object[][] total = new Object[lineArr.length][4];
        for(int i = 0; i<lineArr.length;i++){
            total[i][0] = lineArr[i];
            total[i][1] = data[i][1];
            total[i][2] = data1[i][1];
        ...
        for(int i = 0; i<lineArr.length;i++){
            sumCs += Integer.parseInt(data[i][1].toString());
            sumCt += Integer.parseInt(data1[i][1].toString());
            //total[i][3] = valueArray.get(i);
        }
        jLabel5.setText(String.valueOf(sumCs));
        jLabel3.setText(String.valueOf(sumCs + sumCt));
    ...
    private void btnNestedActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNestedActionPerformed
        ...
        data2 = new Object[lineArr.length][2];
        Object[] columnNames = { "Code Line", "Cnc Value"};
        for(int i = 0; i<lineArr.length;i++){
            data2[i][0] = lineArr[i];
            data2[i][1] = valueArray.get(i);
        }
